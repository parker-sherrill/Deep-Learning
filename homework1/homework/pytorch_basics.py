...
class PyTorchBasics:
    ...
    def make_it_pytorch_4(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.sum(y.flatten() > torch.mean(x.flatten()))
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.sum(torch.any(torch.abs(x.unsqueeze(1) - y.unsqueeze(0)) < 1e-3, dim=1))
